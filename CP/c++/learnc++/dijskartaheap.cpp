...
void addedge(int source,int destination,int len)
 ...
 void printdistance()
 {
     for(int i=0;i<v;i++)
     {
         cout<<"dist of "<<i<<"th vertex is:"<<arr[i].key<<endl;
     }
 }
...
while(h.h_size!=0)
{
    ...
    {
        if(it->dest!=u){if((!visited[it->dest])&& ((arr[u].key+it->length)<(arr[it->dest].key)))

                    {arr[it->dest].key=arr[u].key+it->length ;

                    h.decrease_key(h.pos[arr[it->dest].index],arr[it->dest].key);
                    }
                    }
                    else{if((!visited[it->src])&& ((arr[u].key+it->length)<(arr[it->src].key)))
                    {arr[it->src].key=arr[u].key+it->length ;
