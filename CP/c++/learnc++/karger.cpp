...
{ srand(time(0));
	...
	{
        cout<<"contracting "<<edge[i].src<<" and "<<edge[i].dest<<endl;
		vertices--;
...
int minicut=INT_MAX,i,temp;
for ( i=0;i<V*V*log(V);i++)
{
    temp=kargerMinCut(graph);
    cout<<endl;
    if(minicut>temp)
           minicut=temp;
}
cout<<"the minimum cut is"<<minicut;
	return 0;
}
